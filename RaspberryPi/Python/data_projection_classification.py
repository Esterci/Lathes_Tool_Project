...
def dynamic_tsfresh ():
    ...
    ipython_path = raspberry_path + '/IPython'
    Input_path = ipython_path + add_path1
    Kernel_path = ipython_path + add_path2
    ...
    all_files = glob.glob(Input_path + '/new_data_*.csv')
    latest_csv = max(all_files, key=os.path.getctime)
    Data = np.genfromtxt(latest_csv.format(Input_path), delimiter=',')
    data_frame = pd.DataFrame(Data[:,0:8], columns= ['id','time'] + ['Sensor_' + str(x) for x in range(1,(Data.shape[1]-2))])
...
def PCA_projection (features):
    ...
    raspberry_path = os.path.dirname(os.path.dirname(__file__))
    ipython_path = raspberry_path + '/IPython'
    Kernel_path = ipython_path + add_path1
